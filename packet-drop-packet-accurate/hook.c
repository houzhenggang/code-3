...
// AppEx
// It is so ugly..., 1,2,6,7,3,4,5,10,8,9,(11....)
#if 1
...
// WAN_SINA
// It is so ugly..., 1,2,6,7,3,4,5,9,8(11....)
#if 0
static int __out_order_index[9] = {0,1,4,5,6,2,3,8,7};
static struct sk_buff *__skb_ac[9];
#endif
...
static int handshake_retransmit(struct tcphdr *th)
{
...
static void local_in_time_out(unsigned long data)
{
	int index = 0;
	struct sk_buff *skb;
...
static int make_out_order(struct sk_buff *skb, int (*okfn)(struct sk_buff *))
{
	...
	if (rcv_index < ARRAY_SIZE(__skb_ac)) {
		...
		if (!rcv_index) {
			...
			local_in_timer.function = local_in_time_out;
...
static int establish_retransmit(struct tcphdr *th)
{
	//if (__drop_type == __DROP_CONFIRM_DATA)
	if (__drop_index)
		return __drop_confirm_data(th);

	//if (__drop_type == __DROP_MULTI_DATA)
...
static unsigned int local_in_hook_func(unsigned int hook, 
		struct sk_buff *skb, 
		...
		int (*okfn)(struct sk_buff *))
{
	...
	if (pskb_may_pull(skb, sizeof(struct iphdr))) {
		...
		if (__drop_type & __DROP_SYN_MASK) {
			...
			if (!__dir && (iph->saddr != __src_ip || (iph->daddr != __dst_ip && iph->daddr != __dst_ip2)))
				RETURN(NF_ACCEPT);
			if (__dir && ((iph->saddr != __dst_ip && iph->saddr != __dst_ip2) || iph->daddr != __src_ip))
				RETURN(NF_ACCEPT);
		}
		...
		if (pskb_may_pull(skb, (iph->ihl << 2) + sizeof(struct tcphdr))) {
			...
			if (__drop_type & __DROP_SYN_MASK)
				return handshake_retransmit(th);
			else if (__drop_type == __MAKE_OUT_ORDER && data_len)
				return make_out_order(skb, okfn);
			else if (data_len)
				return establish_retransmit(th);
		}
	}
	RETURN(NF_ACCEPT);
}

static unsigned int local_out_hook_func(unsigned int hook, \
		struct sk_buff *skb, 
		const struct net_device *in, const struct net_device *out, 
		int (*okfn)(struct sk_buff *))
{
